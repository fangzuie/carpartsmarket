...
<table class="table">
    <thead>
      ...
      $sql = "select * from cp_orderdetail INNER JOIN cp_product ON cp_orderdetail.pd_id = cp_product.pd_id INNER JOIN cp_order ON cp_orderdetail.order_id = cp_order.order_id INNER JOIN cp_user ON cp_order.user_id = cp_user.user_id
                    WHERE cp_product.user_id = '$userid' order by '$orderby'";
    ...
    <tbody>
      <tr>
        ...
        <?php if($result['order_status'] == "โอนเงินเรียบร้อย" OR $result['oddt_status'] == "รอจัดส่ง") { ?><td><a class="btn btn-success" href="confirmdeliver.php?oddtid=<?php echo $result['oddt_id']; ?>">ยืนยันการส่งสินค้า</a></td><?php } ?>
      </tr>
    </tbody>
    <?php } ?>
    <?php } else { ?>
