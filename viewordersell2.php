...
$txtUsername = $_GET["txtUsername"];
  ...
  if(isset($result2)) { ?>
    <h4>เรียงตาม  <select onchange="location = this.value;">
      ...
      <option value="viewordersell3.phporderby=oddt_id desc&oddtstatus=ได้รับสินค้า&txtUsername=<?php echo $txtUsername;?>">ได้รับสินค้า</option>
...
<table class="table">
    <thead>
      ...
      $sql = "select * from cp_orderdetail INNER JOIN cp_product ON cp_orderdetail.pd_id = cp_product.pd_id INNER JOIN cp_order ON cp_orderdetail.order_id = cp_order.order_id INNER JOIN cp_user ON cp_order.user_id = cp_user.user_id
                    WHERE cp_product.user_id = '$userid' AND cp_user.user_fullname Like '%$txtUsername%' order by '$orderby'";
