        ...
        <div>
          ...
          <div class="row">
            ...
            <table class="table">
                ...
                <?php $sql = "SELECT * FROM cp_order INNER JOIN cp_user ON cp_order.user_id = cp_user.user_id order by order_id desc";
                ...
                <tbody>
                  <tr>
                    ...
                    <?php if($result['order_status'] == 'รอยืนยันการโอน' /*or $result['order_status'] == 'โอนเงินเรียบร้อย'*/) { ?>
                    <td><a href="pic\slip\<?php echo $result['order_image']; ?>"><?php echo $result['order_status']; ?></a></td>
                    <?php } else { ?>
                      <td><?php echo $result['order_status']; ?></td>
                      <?php } ?>
                    <td><a class="btn btn-success" href="confirmslip.php?orderid=<?php echo $result['order_id']; ?>&orderimage=<?php echo $result['order_image']; ?>" onclick="return confirm('โอนเงินแล้วเรียบร้อยใช่หรือไม่');">ยืนยันสถานะการโอนเงิน</a> <a class="btn btn-danger" href="confirmsliperror.php?orderid=<?php echo $result['order_id']; ?>&orderimage=<?php echo $result['order_image']; ?>" onclick="return confirm('โอนเงินไม่เรียบร้อยใช่หรือไม่');">แจ้งการโอนเงินไม่ถูกต้อง</a></td>
                  </tr>
                </tbody>
                <?php } ?>
              </table>
