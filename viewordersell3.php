...
$oddtstatus = $_GET["oddtstatus"];
  ...
  if(isset($result2)) { ?>
    <h4>เรียงตาม  <select onchange="location = this.value;">
      <option></option>
      <option value="viewordersell3.php?orderby=oddt_id desc&oddtstatus=รอจัดส่ง&txtUsername=<?php echo $txtUsername;?>">รอจัดส่ง</option>
      <option value="viewordersell3.php?orderby=oddt_id desc&oddtstatus=จัดส่งแล้ว&txtUsername=<?php echo $txtUsername;?>">จัดส่งแล้ว</option>
      <option value="viewordersell3.php?orderby=oddt_id desc&oddtstatus=ได้รับสินค้า&txtUsername=<?php echo $txtUsername;?>">ได้รับสินค้า</option>
      <option value="viewordersell3.php?orderby=oddt_id desc&oddtstatus=ไม่ได้รับสินค้า&txtUsername=<?php echo $txtUsername;?>">ไม่ได้รับสินค้า</option>
      <option value="viewordersell.php?orderby=oddt_id desc">ทั้งหมด</option>
    </select></h4>
    <form action="viewordersell2.php" method="GET">
    <div class="form-group col-md-2">
      <input name="txtUsername" id="txtUsername" type="text" value="<?php echo $txtUsername;?>" class="form-control">
...
<table class="table">
    <thead>
      ...
      <?php
      $sql = "select * from cp_orderdetail INNER JOIN cp_product ON cp_orderdetail.pd_id = cp_product.pd_id INNER JOIN cp_order ON cp_orderdetail.order_id = cp_order.order_id INNER JOIN cp_user ON cp_order.user_id = cp_user.user_id
                    WHERE cp_product.user_id = '$userid' AND cp_user.user_fullname Like '%$txtUsername%' AND oddt_status = '$oddtstatus' order by '$orderby'";
