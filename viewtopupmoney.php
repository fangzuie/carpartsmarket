...
<table class="table">
    ...
    <?php $sql2 = "select * from cp_reporttopupmoney where user_id = '".$_SESSION["user_id"]."' order by report_id desc";
    ...
    <tbody>
      <tr>
        ...
        <td><?php if($result['report_status'] == 'โอนเงินไม่ถูกต้อง' ) { ?><a class="btn btn-success" href="topupmoney2.php?reportid=<?php echo $result['report_id']; ?>">ยืนยันการโอนใหม่</a> <?php } ?></td>
      </tr>
    </tbody>
    <?php } ?>
    <?php } else { ?>
      <h1>ยังไม่มีรายการเติมเงิน</h1>
  <?php } ?>
</table>
